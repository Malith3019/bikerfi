...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	// Validate inputs
	if ($loanId > 0 && ($status == "Level One Approved" || $status == "Rejected")) {
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			if ($loanId > 0 && ($status == "Level One Approved" || $status == "Rejected")) {
				...
				if ($result) {
					...
					header("Location: eligibleloans.php?message=Loan application status updated successfully&type=success");
					exit();
				} else {
					// Redirect with error message
					header("Location: eligibleloans.php?message=Error updating loan status&type=error");
					exit();
				}
			} else {
				// Redirect with validation error message
				header("Location: eligibleloans.php?message=Invalid input. Please try again.&type=error");
				exit();
			}
		}
	}
...
<body>
	...
	<aside id="sidebar" class="sidebar">
		<ul class="sidebar-nav" id="sidebar-nav">
			...
			<li class="nav-item"><a class="nav-link  " href="eligibleloans.php">
					<i class="bi bi-arrow-right-square-fill"></i>
					<span>ELIGIBLE LOAN APPLICATIONS</span></a>
			</li>
			<li class="nav-item"><a class="nav-link collapsed" href="rejectedloans.php">
					<i class="bi bi-arrow-right-square-fill"></i>
										...
										<tbody>
											...
											if (mysqli_num_rows($result) > 0) {
												while ($row = mysqli_fetch_assoc($result)) {
													...
													echo "<td>";
													if ($proofAdded == 'Y') {
														echo "<a href='view_proof_documents.php?id=$customer_id' class='btn btn-warning btn-sm' target='_blank'>View Proof</a> ";
													} else {
						...
						<form id="approveLoanForm" action="" method="POST">
							...
							<div class="mb-3">
								...
								<select class="form-control" id="approveStatus" name="status">
									<option value="Level One Approved">Level One Approved</option>

...
// Check if NIC exists in the CRIB table and fetch due_duration
$crib_query = "SELECT due_duration FROM crib WHERE nic = '$nic'";
$crib_result = mysqli_query($conn, $crib_query);

if ($crib_result && mysqli_num_rows($crib_result) > 0) {
    $crib_data = mysqli_fetch_assoc($crib_result);
    $due_duration = $crib_data['due_duration'];

    // If due_duration is greater than 90 days, reject the application
    if ($due_duration > 90) {
        $status = 'REJECTED';
        $credit_score = 0; // Set credit score to 0 since it's rejected
		$customer_id = $id;
        ...
        if (mysqli_query($conn, $sql)) {
            $scoree = "Your loan application is rejected due to outstanding dues exceeding 90 days in the CRIB. Credit Score: $credit_score";
...
// If due_duration is not greater than 90, continue with credit score calculation

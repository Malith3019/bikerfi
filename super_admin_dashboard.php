...
if (isset($_POST['submit'])) {
	...
	if (mysqli_num_rows($check_result) > 0) {
		header("Location: super_admin_dashboard.php?error=Email or NIC already exists for another user.");
		exit();
	} else {
		// Handle file upload
		if (!empty($_FILES['profilePic']['name'])) {
			...
			if (!in_array($file_extension, $allowed_extensions)) {
				header("Location: super_admin_dashboard.php?error=Only JPG, JPEG, PNG, and WEBP files are allowed.");
				exit();
			} elseif ($_FILES["profilePic"]["size"] > 15000000) { // 15MB max file size
				header("Location: super_admin_dashboard.php?error=File size should be less than 15MB.");
				exit();
			} else {
				...
				if (move_uploaded_file($_FILES["profilePic"]["tmp_name"], $target_file)) {
					...
					header("Location: super_admin_dashboard.php?error=Error uploading the image.");
					exit();
				}
			}
		}
		...
		if (mysqli_stmt_execute($update_stmt)) {
			header("Location: super_admin_dashboard.php?success=Profile updated successfully.");
			exit();
		} else {
			header("Location: super_admin_dashboard.php?error=Error updating profile.");
			exit();
		}
	}
}
						...
						<div class="card-body">
							...
							<form method="post" action="" enctype="multipart/form-data">
								...
								<div class="row mb-3">
									...
									<div class="col-sm-10">
										<input type="text" id="nic" name="nic" class="form-control" value="<?= $nic; ?>" readonly>
									</div>
								</div>

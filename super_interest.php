...
// Handle score update
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_intrest'])) {
	$id = $_POST['id'];
	$tenture = mysqli_real_escape_string($conn, $_POST['tenture']);
	$rate = mysqli_real_escape_string($conn, $_POST['rate']);
	 

	$sql = "UPDATE loan_interest_rates SET loan_tenure='$tenture', interest_rate='$rate' WHERE id=$id";
	$result = mysqli_query($conn, $sql);

	if ($result) {
		header("Location: super_interest.php?message=Score updated successfully&type=success");
	} else {
		header("Location: super_interest.php?message=Error updating score&type=error");
	}
	exit();
}
...
<body>
	...
	<aside id="sidebar" class="sidebar">
		<ul class="sidebar-nav" id="sidebar-nav">
			...
			<li class="nav-item"><a class="nav-link  " href="super_interest.php">
					<i class="bi bi-arrow-right-square-fill"></i>
					...
					<div class="row">
						<div class="col-12">
							<div class="card recent-sales overflow-auto">
								<div class="card-body">
									<h5 class="card-title">MENU > INTREST RATES</h5>
									<table class="table table-bordered custom-gray-table table-striped datatable" id="creditScoreTable">
										...
										<tbody>
											<?php
											$sql = "SELECT id, loan_tenure, interest_rate FROM loan_interest_rates";
											$result = mysqli_query($conn, $sql);
											while ($row = mysqli_fetch_assoc($result)) {
												...
												echo "<td><button class='btn btn-primary btn-edit' data-id='{$row['id']}' data-tenture='{$row['loan_tenure']}' data-rate='{$row['interest_rate']}' data-bs-toggle='modal' data-bs-target='#editModal'>Edit</button></td>";
												echo "</tr>";
											}
											?>
										</tbody>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Edit Rates</h5>
			...
			<div class="modal-body">
				<form method="POST">
					...
					<div class="mb-3">
						<label for="tenture" class="form-label">Loan Tenture</label>
						<input type="text" class="form-control" name="tenture" id="tenture" required>
					</div>
					<div class="mb-3">
						<label for="rate" class="form-label">Intrest Rate</label>
						<input type="text" class="form-control" name="rate" id="rate" required>
					</div>
					<button type="submit" name="update_intrest" class="btn btn-success">Update</button>
				</form>
			</div>
		</div>
	</div>
	...
	document.addEventListener("DOMContentLoaded", function() {
		...
		editButtons.forEach(button => {
			button.addEventListener("click", function() {
				...
				document.getElementById("tenture").value = this.getAttribute("data-tenture");
				document.getElementById("rate").value = this.getAttribute("data-rate");
 			});
		});
	});
</script>
